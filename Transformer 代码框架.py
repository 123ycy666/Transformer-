...
if __name__ == "__main__":
    ...
    dummy_mask = torch.ones(batch_size, seq_len, seq_len) # 简单的全1掩码

    print("测试位置编码...")
    pos_out = pos_encoder(dummy_input)
    print(f"位置编码输出形状: {pos_out.shape}")

    print("测试编码器层...")
    enc_out = encoder_layer(dummy_input, dummy_mask)
    print(f"编码器层输出形状: {enc_out.shape}")

    print("测试解码器层...")
    dec_out = decoder_layer(dummy_input, enc_out, dummy_mask, dummy_mask)
    print(f"解码器层输出形状: {dec_out.shape}")
